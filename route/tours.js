...
router.delete("/Mytours/delete/:id",verifyUser,function (req, res) {
  ...
  //update ++
  router.put('/tours/add/:id', function(req,res){
    const id = req.params.id;
  
    MyTours.findOne({_id : id}).then(function(result) {
      let person = result.Person; 
      MyTours.findByIdAndUpdate({_id: id}, {$set:{
        Person : person+1
      }}).then(function(result){
  ...
  //update --
  router.put('/tours/minus/:id', function(req,res){
    const id = req.params.id;
